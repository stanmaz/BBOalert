...
function myDirection() {
	if (parent.document.title == "RealBridge") {
		var svb = parent.window.document.getElementById("self_vul_box");
		if (svb == null) return '';
		return svb.innerText;
	}
...
function mySeat() {
	if (parent.document.title == "RealBridge") {
		var svb = parent.window.document.getElementById("self_vul_box");
		if (svb == null) return '';
		return svb.innerText.substring(0, 1);
	}
	if ((nd = getNavDiv()) == null) return '';
...
function accountSettingsSet(idx) {
	...
	if (rd == null) return '';
	var sc = rd.querySelectorAll('.setting-class');
	if (sc.length < 6) {
		if (sc.length == 0) return '';
	}
	try {
		if (parent.document.querySelectorAll('.setting-class')[idx].querySelector('ion-toggle').getAttribute("aria-checked") == "true") return 'Y';
		else return 'N';
	} catch {
		return '';
	}
...
function addBBOalertTab() {
	if (parent.document.title == "RealBridge") {
		rd = parent.document.body;
		//		rd = parent.document.getElementById("self_name_container");
		if (rd == null) return;
		t = document.createElement("button");
		t.textContent = "BBOalert";
		t.id = 'bboalert-tab';
		t.style.color = 'white';
		...
		t.style.bottom = 0;
		t.style.zIndex = 5001;
		t.style.fontSize = parent.window.document.querySelector("#rho_chat_bar input").style.fontSize;
...
function areWeVulnerable() {
	if (parent.document.title == "RealBridge") {
		var svb = parent.window.document.getElementById("self_vul_box");
		if (svb == null) return '';
		if (svb.style.backgroundColor == "rgb(255, 255, 255)") return "@n";
		return ("@v");
	}
	if ((nd = getNavDiv()) == null) return '';
	var cells = nd.querySelectorAll('.auction-box-header-cell');
...
function areTheyVulnerable() {
	if (parent.document.title == "RealBridge") {
		var lvb = parent.window.document.getElementById("lho_vul_box");
		if (lvb == null) return '';
		if (lvb.style.backgroundColor == "rgb(255, 255, 255)") return "@N";
		return ("@V");
	}
	if ((nd = getNavDiv()) == null) return '';
	var cells = nd.querySelectorAll('.auction-box-header-cell');
...
function getDealNumber() {
	if (parent.document.title == "RealBridge") {
		var nb = parent.window.document.getElementById("number_box");
		if (nb == null) return '';
		return nb.innerText;
	}
...
function getCurrentChatDestination() {
	var mi = $('#chatDiv .channelButtonClass', parent.window.document);
...
function getChatDestinationMenuItem(t) {
	var mi = $('#chatDiv menu-item div', parent.window.document);
...
function isChatDestinationOK(t) {
	var cb = $('#chatDiv .messageInputDivClass .channelButtonClass', parent.window.document)[0];
...
function setChatDestination(t) {
	...
	var cb = $('#chatDiv .messageInputDivClass .channelButtonClass', parent.window.document)[0];
	var ok = false;
	$('#chatDiv .menuClass', parent.window.document).hide();
	$('#chatDiv .messageInputDivClass .channelButtonClass', parent.window.document)[0].click();
...
function sendPrivateChat(uid, text) {
	var t = text;
	if (!t.endsWith('\\n')) t = t + '\\n';
	var od = $('#chatDiv .messageInputDivClass .channelButtonClass', parent.window.document)[0].textContent;
	setChatDestination('Private');
	//    var cd = $('#chatDiv .messageInputDivClass .channelButtonClass span');
	var cd = $('#chatDiv .getStringDialogClass .messageInputClass', parent.window.document);
	var bt = $('#chatDiv .getStringDialogClass button', parent.window.document);
	setTimeout(function () {
		cd[0].value = uid;
		var eventInput = new Event('input');
...
function getMyHand() {
	var yref = 0;
	var hand = '';
	var cc = $('#navDiv .cardClass .topLeft', parent.window.document);
...
function getPartnerHand() {
	var yref = 1000000;
	var hand = '';
	var cc = $('.cardClass .topLeft', parent.window.document);
	if (cc.length == 0) return '';
	for (var i = 0; i < cc.length; i++) {
		if (cc[i].getClientRects().length > 0) {
			var card = cc[i].parentNode.parentNode;
...
function getAuctionBox() {
	try {
		return $('bridge-screen .auctionBoxClass', parent.window.document)[0];
	} catch {
		return null;
	}
}
...
function getCard(index) {
	var card = $(".cardClass", parent.window.document).filter(function () {
...
function getLastChatMessaage() {
	try {
		var ci = $("#chatDiv .chatOutputClass chat-list-item", parent.window.document).toArray();
...
function getAnnouncementPanel() {
	return $("bridge-screen .announcementPanelClass", parent.window.document)[0];
}

function getNotificationPanel() {
	return $("bridge-screen .notificationClass", parent.window.document)[0];
}

function getCallExplanationPanel() {
	return $("bridge-screen .callExplanationClass", parent.window.document)[0];
}

function getCallExplanationText() {
	return $("bridge-screen .callExplanationClass .textClass", parent.window.document).text();
}
...
function hover_bboalert() {
	try {
		...
		if (t.onmouseenter == null) t.onmouseenter = function () {
			if (isHoverEnabled()) {
				setOptions(true);
				$("#bboalert-tab", parent.window.document)[0].focus();
			}
		};
		for (var i = 0; i < tabs.length; i++) {
			if (tabs[i].textContent.search('BBOalert') == -1) {
...
function getFinalContractPanel() {
	try {
		return $("bridge-screen .tricksPanelClass", parent.window.document)[0];
	} catch {
		return null;
	}
}
...
function RBaddTextarea() {
//	var pc = parent.window.document.getElementById("contract_container");
	var pc = parent.window.document.body;
	if (pc == null) return false;
	let ta = parent.window.document.getElementById("bboalert-textarea");
	if (ta != null) return false;
	ta = document.createElement("input");
	ta.id = "bboalert-textarea";
	ta.type = "text";
	ta.style.position = "absolute";
	ta.style.bottom = "0px";
	ta.style.left = "0px";
	ta.style.fontSize = parent.window.document.querySelector("#rho_chat_bar input").style.fontSize;
	ta.style.width = "90%";
	ta.style.zIndex = 4999;
	ta.placeholder = "BBOalert text";
	ta.oninput = function () {
		let ae = parent.window.document.getElementById("alert_explain");
		if (ae == null) return;
		ae.value = this.value.replaceAll("\n", " ");
		ae.dispatchEvent(new Event('input'));
		let lcbi = parent.window.document.querySelector("#lho_chat_bar input");
		lcbi.value = RBgetCall() + " = " + ae.value;
		lcbi.dispatchEvent(new Event('input'));
		let rcbi = parent.window.document.querySelector("#rho_chat_bar input");
		rcbi.value = RBgetCall() + " = " + ae.value;
		rcbi.dispatchEvent(new Event('input'));
	}
	pc.appendChild(ta);
}

function RBremoveTextarea() {
	var ta = parent.window.document.getElementById("bboalert-textarea");
	if (ta != null) {
		ta.value == "";
